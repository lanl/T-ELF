...
import scipy
import scipy.sparse
from TELF.factorization.decompositions import nmf_fro_admm
...
def test_H_update_numpy():
    ...
    for dtype in [np.float32, np.float64]:
        for typ in [np.array, scipy.sparse.csc_matrix, scipy.sparse.csr_matrix, scipy.sparse.coo_matrix]:
            ...
            H = nmf_fro_admm.H_update(X, W0, uniform_product(H0, 0.1), use_gpu=False)
...
def test_W_update_numpy():
    ...
    for dtype in [np.float32, np.float64]:
        for typ in [np.array, scipy.sparse.csc_matrix, scipy.sparse.csr_matrix, scipy.sparse.coo_matrix]:
            X = typ(X0.astype(dtype))
            W = nmf_fro_admm.W_update(X, uniform_product(W0, 0.1), H0, use_gpu=False)
...
def test_nmf_numpy():
    ...
    for dtype in [np.float32, np.float64]:
        for typ in [np.array, scipy.sparse.csc_matrix, scipy.sparse.csr_matrix]:
            X = typ(X0.astype(dtype))
            W, H = nmf_fro_admm.nmf(X, uniform_product(
...
def test_H_update_cupy():
    ...
    for dtype in [np.float32, np.float64]:
        for typ in [cp.array, cupyx.scipy.sparse.csc_matrix, cupyx.scipy.sparse.csr_matrix]:
            X = typ(X0.astype(dtype))
            H = nmf_fro_admm.H_update(X, W0, uniform_product(H0, 0.1, use_gpu=True), use_gpu=True)
            assert H.dtype == dtype
...
def test_W_update_cupy():
    ...
    for dtype in [np.float32, np.float64]:
        for typ in [cp.array, cupyx.scipy.sparse.csc_matrix, cupyx.scipy.sparse.csr_matrix]:
            X = typ(X0.astype(dtype))
            W = nmf_fro_admm.W_update(X, uniform_product(W0, 0.1, use_gpu=True), H0, use_gpu=True)
            assert W.dtype == dtype
