...
import scipy
import scipy.sparse
from TELF.factorization.decompositions import nmf_kl_admm
...
def test_H_update_numpy():
    ...
    for dtype in [np.float32, np.float64]:
        for typ in [np.array, scipy.sparse.csc_matrix, scipy.sparse.csr_matrix]:
            ...
            H = nmf_kl_admm.H_update(X, W0, uniform_product(H0, 0.1), use_gpu=False)
...
def test_W_update_numpy():
    ...
    for dtype in [np.float32, np.float64]:
        for typ in [np.array, scipy.sparse.csc_matrix, scipy.sparse.csr_matrix]:
            X = typ(X0.astype(dtype))
            W = nmf_kl_admm.W_update(X, uniform_product(W0, 0.1), H0, use_gpu=False)
